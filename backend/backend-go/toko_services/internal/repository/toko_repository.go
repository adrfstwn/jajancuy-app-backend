...
func (r *TokoRepository) CreateToko(toko domain.Toko) (int, error) {
	query := "INSERT INTO toko (name, description, owner_id) VALUES ($1, $2, $3) RETURNING id"
	var id int
	err := r.db.QueryRow(query, toko.Name, toko.Description, toko.OwnerID).Scan(&id)
	if err != nil {
		return 0, err
	}
	return id, nil
...
func (r *TokoRepository) GetTokoByID(id int) (*domain.Toko, error) {
	query := "SELECT id, name, description, owner_id FROM toko WHERE id = $1"
	row := r.db.QueryRow(query, id)

	var toko domain.Toko
	err := row.Scan(&toko.ID, &toko.Name, &toko.Description, &toko.OwnerID)
	if err != nil {
...
func (r *TokoRepository) GetAllToko() ([]domain.Toko, error) {
	query := "SELECT id, name, description, owner_id FROM toko"
	...
	for rows.Next() {
		var toko domain.Toko
		err := rows.Scan(&toko.ID, &toko.Name, &toko.Description, &toko.OwnerID)
		if err != nil {
			return nil, err
		}
...
func (r *TokoRepository) UpdateToko(toko domain.Toko) error {
	...
	_, err := r.db.Exec(query, toko.Name, toko.Description, toko.OwnerID, toko.ID)
	return err
}
